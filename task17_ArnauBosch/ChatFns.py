...
def LoadOtherEntry(ChatLog, EntryText, uname):
    if EntryText != '':
        ChatLog.config(state=NORMAL)
        if ChatLog.index('end') != None:
            try:
                ...
                pass
            uname = uname[:-1]
            ChatLog.insert(END, uname + ": " + EntryText)
            ChatLog.tag_add(uname, LineNumber, LineNumber+float("0."+str(len(uname))))
            ChatLog.tag_config(uname, foreground="#04B404", font=("Arial", 12, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)